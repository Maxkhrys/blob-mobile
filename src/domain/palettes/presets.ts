...
export function getCloudPresetById(id: string): CloudColourPreset {
  ...
  if (norm === 'white' || norm === 'cloud-white') return CANONICAL_CLOUD_PRESETS[0];
  if (norm === 'blue' || norm === 'cloud-blue') return CANONICAL_CLOUD_PRESETS[1];
  ...
  return (
    CANONICAL_CLOUD_PRESETS.find((p) => p.id === id || p.name.toLowerCase() === norm) ||
